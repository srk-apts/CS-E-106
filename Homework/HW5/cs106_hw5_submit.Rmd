...
```{r}
X_0 = rep_len(1, nrow(plastic_data))
X_1 = plastic_data$X
X = cbind(X_0, X_1)
```
...
Based on the regression function, none of the $\beta$'s seem to be zero. $\beta_{2}$ has a higher standard error and a greater p-value, which means X1 is more correlated to Y compared to X2. Also, the model is a very good fit ($R^2=0.95$).
...
```{r}
Xh<-data.frame(X1=5, X2=4)
predict(lm_brand, Xh,se.fit=TRUE,interval="prediction",level=1-alpha)
```
...
- For X2 and X4 and their interactions, the error variance is constant implying a linear relation with Y.
